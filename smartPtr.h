...
// }
    ...
    private:
        ...
        {
            if(refcount!=nullptr) refcount->__IncRefCount();
            IRefCount<T> *oldref = __m_refcount;
            __m_refcount = refcount;
            if(oldref!=nullptr) oldref->__DecRefCount();
        }
    public:
        SmartPtr()
        {
            __m_refcount = nullptr;
        }
        SmartPtr(T * ptr)
        {
            __m_refcount = nullptr;
        ...
        T *GetPtr() const
        {
            if(__m_refcount==nullptr) return nullptr;
